...
public class RemoteProductDataSource implements ProductDataSource {
    ...
    @NonNull
    private Function<ListingResponse, List<ProductDetail>> mProductMapperFunction;

    @Inject
    public RemoteProductDataSource() {
        RedMartApplication.getApplicationComponent().inject(this);
        mProductMapperFunction = this::getProductFromResponse;
    }
    ...
    public Observable<List<ProductDetail>> getProducts(int pageNo) {
        return retrofitProductData.requestProductListObservable(pageNo, AppConstants.MAX_ITEMS_PER_REQUEST).map(mProductMapperFunction);
    }

    @Override
    public Observable<ProductDetail> getProduct(@NonNull String productId) {
        return retrofitProductData.requestProductObservable(productId);
    }
